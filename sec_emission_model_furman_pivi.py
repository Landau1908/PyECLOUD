...
class SEY_model_furman_pivi:
    ...
    def __init__(
        ...
        self.use_ECLOUD_theta0_dependence = furman_pivi_surface[
            "use_ECLOUD_theta0_dependence"
        ]
        ...
        if self.exclude_rediffused:
            print(
                (
                    "Secondary emission model: Furman-Pivi excluding rediffused, s=%.4f"
                    % (self.s)
                )
            )
        else:
    ...
    def SEY_process(self, E_impact_eV, costheta_impact, i_impact):
        ...
        delta_e, delta_r, delta_ts = self.yield_fun_furman_pivi(
            E_impact_eV, costheta_impact
        )
    ...
    def delta_e(self, E_impact_eV, costheta_impact):
        ...
        if self.flag_costheta_delta_scale:
            if self.use_ECLOUD_theta0_dependence:
                ...
                angular_factor = 1.0 + self.e1 * (1.0 - costheta_impact ** self.e2)
        else:
            angular_factor = 1
    ...
    def delta_r(self, E_impact_eV, costheta_impact):
        ...
        delta_r0 = self.p1RInf * (1.0 - np.exp(exp_factor))
        if self.flag_costheta_delta_scale:
            angular_factor = 1.0 + self.r1 * (1.0 - costheta_impact ** self.r2)
        else:
            angular_factor = 1
    ...
    def delta_ts(self, E_impact_eV, costheta_impact):
        ...
        if self.flag_costheta_Emax_shift:
            eHat = self.eHat0 * (1.0 + self.t3 * (1.0 - costheta_impact ** self.t4))
        else:
            eHat = self.eHat0
        delta_ts0 = self.deltaTSHat * self._D(E_impact_eV / eHat)
        if self.flag_costheta_delta_scale:
            if self.use_ECLOUD_theta0_dependence:
                angular_factor = np.exp(0.5 * (1.0 - costheta_impact))
            else:
                angular_factor = 1.0 + self.t1 * (1.0 - costheta_impact ** self.t2)
        else:
            angular_factor = 1
    ...
    def get_energy_backscattered(self, E_0):
        ...
        if self.use_modified_sigmaE:
            ...
            return E_0 + sqrt2 * sigmaE_modified * erfinv(
                (uu - 1) * erf(E_0 / (sqrt2 * sigmaE_modified))
            )
        else:
            return E_0 + sqrt2 * self.sigmaE * erfinv(
                (uu - 1) * erf(E_0 / (sqrt2 * self.sigmaE))
            )
    ...
    def get_energy_true_sec(self, nn, E_0):
        ...
        if self.use_ECLOUD_energy:
            Ngen = len(E_0)
            return ee.sec_energy_hilleret_model2(
            ...
            _, area = self._true_sec_energy_CDF(
                nn, energy=E_0
            )  # Putting energy=E_0 gives area under the PDF
            normalisation = 1.0 / area
            uu = random.rand(len(E_0))
            xx = uu / (normalisation)

            xx[
                xx < 1e-12
            ] = 0.0  # gammaincinv returns nan if xx is too small but not zero
    ...
    def impacts_on_surface(
        ...
        (
            ...
            delta_r,
            delta_ts,
        ) = self.SEY_process(E_impact_eV, costheta_impact, i_found)
        ...
        if self.use_ECLOUD_energy:
            (
                vx_replace[flag_backscattered],
                vy_replace[flag_backscattered],
            ) = ee.specular_velocity(
                vx_impact[flag_backscattered],
                vy_impact[flag_backscattered],
                Norm_x[flag_backscattered],
                Norm_y[flag_backscattered],
                v_impact_n[flag_backscattered],
            )
        else:
            En_backscattered_eV = self.get_energy_backscattered(
                E_impact_eV[flag_backscattered]
            )
            N_backscattered = np.sum(flag_backscattered)
            (
                vx_replace[flag_backscattered],
                vy_replace[flag_backscattered],
                vz_replace[flag_backscattered],
        ...
        if not self.exclude_rediffused:
            ...
            (
                vx_replace[flag_rediffused],
                vy_replace[flag_rediffused],
                vz_replace[flag_rediffused],
            ) = self.angle_dist_func(
                N_rediffused,
                En_rediffused_eV,
                Norm_x[flag_rediffused],
                Norm_y[flag_rediffused],
                mass,
            )
        ...
        if N_true_sec > 0:
            if self.exclude_rediffused:
                ...
                delta_ts_prime = delta_ts[flag_truesec] / (
                    1 - delta_e[flag_truesec] - delta_r[flag_truesec]
                )  # delta_ts^prime in FP paper, eq. (39)

            # Decide how many MPs to be emitted
            n_emit_truesec_MPs[flag_truesec] = random.poisson(
                lam=delta_ts_prime
            )  # Using (45)
            ...
            while Nabove_th > 0:
                n_emit_truesec_MPs_flag_true_sec[flag_above_th] = random.poisson(
                    delta_ts_prime[flag_above_th]
                )
                flag_above_th = n_emit_truesec_MPs_flag_true_sec > self.M_cut
                Nabove_th = np.sum(flag_above_th)
            ...
            # MPs to be replaced
            flag_above_zero = (
                n_emit_truesec_MPs_flag_true_sec > 0
            )  # I exclude the absorbed
            ...
            En_truesec_eV = self.get_energy_true_sec(
                ...
                E_0=E_impact_eV[flag_truesec_and_above_zero],
            )  # First generated MPs
            ...
            if n_add_total != 0:
                ...
                n_emit_truesec_MPs_extended = np.repeat(
                    n_emit_truesec_MPs_flag_true_sec, n_add[flag_truesec]
                )
                En_truesec_eV_add = self.get_energy_true_sec(
                    nn=n_emit_truesec_MPs_extended, E_0=E_impact_eV_add
                )

                # Ensure energy conservation in each event
                if self.conserve_energy:
                    En_truesec_eV_extended = np.repeat(
                        En_truesec_eV, n_add[flag_truesec][flag_above_zero]
                    )  # Energy of replaced MPs
                    E_impact_eV_add = np.repeat(
                        E_impact_eV[flag_truesec][flag_above_zero],
                        n_add[flag_truesec][flag_above_zero],
                    )  # Energy of new MPs
                    En_emit_eV_event_add = (
                        En_truesec_eV_extended + En_truesec_eV_add
                    )  # Total energy emitted in each trusec event

                    flag_violation = (
                        En_emit_eV_event_add > E_impact_eV_add
                    )  # Violation flag for the new MPs
                    flag_violation_replace = self.inverse_repeat(
                        flag_violation,
                        repeats=n_add[flag_truesec][flag_above_zero],
                        axis=None,
                    )  # Violation flag for the replaced MPs
                    ...
                    while N_violations > 0:
                        ...
                        En_truesec_eV[
                            flag_violation_replace
                        ] = self.get_energy_true_sec(
                            nn=n_emit_truesec_MPs_flag_true_sec[flag_above_zero][
                                flag_violation_replace
                            ],
                            E_0=E_impact_eV[flag_truesec][flag_above_zero][
                                flag_violation_replace
                            ],
                        )
                ...
                # Replace velocities
                (
                    vx_replace[flag_truesec_and_above_zero],
                    vy_replace[flag_truesec_and_above_zero],
                    vz_replace[flag_truesec_and_above_zero],
                ) = self.angle_dist_func(
                    N_true_sec,
                    En_truesec_eV,
                    Norm_x[flag_truesec_and_above_zero],
                    Norm_y[flag_truesec_and_above_zero],
                    mass,
                )
                # New velocities
    ...
    def backscattered_energy_CDF(self, energy, E_0, sigma_e=2.0):
        """The CDF for backscattered electrons."""
        sqrt2 = np.sqrt(2)
        return 1 - erf((E_0 - energy) / (sqrt2 * sigma_e)) / erf(
            E_0 / (sqrt2 * sigma_e)
        )
    ...
    def true_sec_energy_PDF(self, delta_ts, nn, E_0, energy):
        ...
        if self.choice == "poisson":
            P_n_ts = np.squeeze(
                delta_ts ** nn_all / factorial(nn_all) * np.exp(-delta_ts)
            )
        elif self.choice == "binomial":
            p = delta_ts / self.M_cut
            P_n_ts = np.squeeze(
                binom(self.M_cut, nn) * (p) ** nn_all * (1 - p) ** (self.M_cut - nn_all)
            )
        else:
    ...
    def average_true_sec_energy_PDF(self, delta_ts, E_0, energy):
        ...
        for ii in nns:
            f_n_ts, P_n_ts = self.true_sec_energy_PDF(
                delta_ts=delta_ts, nn=ii, E_0=E_0, energy=energy
            )
            if False:  # self.conserve_energy:
                ...
                factor = (
                    eps_n_curr ** p_n_curr
                    * gamma(p_n_curr)
                    * gammainc(ii * p_n_curr, E_0 / eps_n_curr)
                )
                factor = 1.0
                if ii - 1 == 0:
                    average_f_n_ts = (
                        average_f_n_ts + f_n_ts * P_n_ts * ii * 1.0 / factor
                    )
                else:
                    average_f_n_ts = (
                        average_f_n_ts
                        ...
                        * P_n_ts
                        * ii
                        * gammainc((ii - 1) * p_n_curr, (E_0 - energy) / eps_n_curr)
                        / factor
                    )
            else:
