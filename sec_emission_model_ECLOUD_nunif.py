...
class SEY_model_ECLOUD_non_unif(SEY_model_ECLOUD):
    def __init__(
        ...
        if chamb.chamb_type != "polyg":
            raise ValueError(
                """ECLOUD_nunif can be used only with chamb_type='polyg'!!!"""
            )
        ...
        self.del_max_segments[chamb.del_max_segments < 0.0] = del_max
        self.R0_segments[chamb.R0_segments < 0.0] = R0
        self.Emax_segments[chamb.Emax_segments < 0.0] = Emax
    ...
    def SEY_process(self, nel_impact, E_impact_eV, costheta_impact, i_impact):
        ...
        yiel, ref_frac = yield_fun2(
            E_impact_eV, costheta_impact, Emax_mp, del_max_mp, R0_mp, E0=self.E0
        )
...
class SEY_model_ECLOUD_non_unif_charging(SEY_model_ECLOUD_non_unif):
    def __init__(
        ...
        super(SEY_model_ECLOUD_non_unif_charging, self).__init__(
            chamb,
            Emax,
            del_max,
            R0,
        ...
        print(
            "Secondary emission model: ECLOUD non uniform E0=%f, with charging"
            % self.E0
        )

        self.chamb = chamb
        self.Q_segments = 0.0 * self.del_max_segments
    ...
    def SEY_process(self, nel_impact, E_impact_eV, costheta_impact, i_impact):
        ...
        if np.any(mask_charging):
            ...
            Q_charging[Q_charging < 0.0] = 0.0
            Q_charging[Q_charging > Q_max] = Q_max[Q_charging > Q_max]

            yiel[mask_charging] = yiel[mask_charging] * (1.0 - Q_charging / Q_max) + (
                1.0 - np.exp(-E_impact_eV[mask_charging] / EQ)
            ) * (Q_charging / Q_max)
        ...
        for i_edg, flag_Q in enumerate(self.flag_charging):
            ...
            if flag_Q:
                ...
                self.Q_segments[i_edg] += (
                    (n_impact_here - n_emit_here) * (-qe) / self.chamb.L_edg[i_edg]
                )

        return nel_emit, flag_elast, flag_truesec

    def SEY_model_evol(self, Dt):
        mask_evol = np.logical_and(self.flag_charging, self.tau_segments > 0.0)
