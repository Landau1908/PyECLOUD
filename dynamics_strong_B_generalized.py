...
class pusher_strong_B_generalized:
    def __init__(self, Dt, B0x, B0y, B_map_file, fact_Bmap, B_zero_thrhld):
    ...
    def step(self, MP_e, Ex_n, Ey_n, Ez_n=0.0, Bx_n=0.0, By_n=0.0, Bz_n=0.0):
        ...
        if MP_e.N_mp > 0:
            ...
            Dz = (
                vzn * sinwcDt / omegac
                + vB_Bmod * coswcDt / omegac
                + EB_Bmodsq * (self.Dt - sinwcDt / omegac)
                - vB_Bmod / omegac
            )

            Dz_tilde = (
                (vzn * (1 - coswcDt) + vB_Bmod * sinwcDt) / (omegac * omegac)
                + EB_Bmodsq
                * (self.Dt * self.Dt / 2.0 + (coswcDt - 1.0) / (omegac * omegac))
                - vB_Bmod * self.Dt / omegac
            )

            vzn1 = vzn * coswcDt - vB_Bmod * sinwcDt + EB_Bmodsq * (1.0 - coswcDt)
            ...
            xn1 = (
                xn
                + vxn * self.Dt
                - qm * (Ex_n * self.Dt * self.Dt / 2.0 - By_n * Dz_tilde)
            )
            yn1 = (
                yn
                + vyn * self.Dt
                - qm * (Ey_n * self.Dt * self.Dt / 2.0 + Bx_n * Dz_tilde)
            )
            zn1 = zn + Dz
            ...
            if N_B_zero > 0:
                ...
                xn1[flag_B_zero] = (
                    xn[flag_B_zero]
                    + vxn[flag_B_zero] * self.Dt
                    - 0.5 * qm * Ex_n[flag_B_zero] * self.Dt * self.Dt
                )
                yn1[flag_B_zero] = (
                    yn[flag_B_zero]
                    + vyn[flag_B_zero] * self.Dt
