...
def assert_module_has_parameters(module, module_name):
    ...
    for attr in dir(module):
        if (not attr.startswith("_")) and (
            not isinstance(getattr(module, attr), types.ModuleType)
        ):
    ...
    allowed_parameters = set.union(
        mandatory_parameters, optional_parameters, parameters_dict["superparameters"]
    )

    extra_parameters = set.difference(module_contents, allowed_parameters)
    if extra_parameters:
        err_msg += "Error! These parameters should not be in %s: %r" % (
            nice_module_name,
            extra_parameters,
        )
    ...
    if missing_parameters:
        err_msg += "\nError! These mandatory parameters are not provided by %s: %r" % (
            nice_module_name,
            missing_parameters,
        )
...
def update_module(module, new_module):
    ...
    for attr in dir(new_module):
        ...
        if attr.startswith("_") or isinstance(value, types.ModuleType):
            ...
            raise PyECLOUD_ConfigException(
                "Parameter %s is specified in %s and %s!"
                % (attr, module.__name__, new_module.__name__)
            )
        else:
...
def update_config_dict(
    config_dict, module, module_name, verbose=False, default_obj=None
):
    ...
    for parameter, default_value in list(
        optional_parameters.items()
    ):  # iterates on keys and values of the dictionary
        ...
        if parameter in config_dict:
            raise PyECLOUD_ConfigException(
                "Parameter %s is specified multiple times!" % parameter
            )
...
def import_module_from_file(module_name, file_name):
    ...
    try:
        new_file_name = dir_name + "/temp_file_%s.py" % (
            module_name + "_" + str(time.time())
        ).replace(".", "_")
        ...
        if sys.version_info.major == 2:
            ...
            spec = importlib.util.spec_from_file_location(
                real_module_name, new_file_name
            )
