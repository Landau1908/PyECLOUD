...
class pyecloud_saver:
    ...
    def start_observing(
        ...
        dec_fact_out=1,
        stopfile="stop",
        flag_multiple_clouds=False,
        cloud_name=None,
        flag_last_cloud=True,
        checkpoint_DT=None,
        ...
        self._energy_cos_and_lifetime_angle_hist_init(
            Dt_En_hist,
            flag_cos_angle_hist,
            cos_angle_width,
            flag_lifetime_hist,
        ...
        if flag_presence_sec_beams:
            ...
            for sec_beam in sec_beams_list:
                self.sec_beam_profiles.append(
                    sec_beam.lam_t_array[::dec_fac_secbeam_prof]
                )
        ...
        if self.extract_sey:
            n_rep = 10000
            self.sey_test_E_impact_eV = np.array(
                list(np.arange(0, 499.0, 1.0)) + list(np.arange(500.0, 2000, 5))
            )
            self.sey_test_cos_theta = np.linspace(0, 1.0, 10)
            self.sey_test_deltas = impact_man.extract_sey_curves(
                n_rep,
                self.sey_test_E_impact_eV,
                self.sey_test_cos_theta,
                MP_e.charge,
                MP_e.mass,
            )
        else:
            self.sey_test_E_impact_eV = 0.0
            self.sey_test_cos_theta = 0.0
            self.sey_test_deltas = {}

        # extract energy distributions
        if self.extract_ene_dist:
            n_rep = int(1e5)
            self.ene_dist_test_cos_theta = np.linspace(0.0, 1.0, 10)
            self.emit_ene_dist_test = impact_man.extract_energy_distributions(
                n_rep,
                self.ene_dist_test_E_impact_eV,
                self.ene_dist_test_cos_theta,
                mass=MP_e.mass,
                Nbin_extract_ene=self.Nbin_extract_ene,
                factor_ene_dist_max=self.factor_ene_dist_max,
            )
        else:
            self.ene_dist_test_cos_theta = 0.0
            self.ene_dist_test_E_impact_eV = 0.0
    ...
    def witness(
        ...
        self._sim_state_save(
            beamtim,
            spacech_ele,
            t_sc_ON,
            flag_presence_sec_beams,
            sec_beams_list,
            self.flag_multiple_clouds,
            cloud_list,
        )
        ...
        if beamtim.flag_new_bunch_pass:
            ...
            sio.savemat(
                self.filen_main_outp, self.build_outp_dict(buildup_sim), oned_as="row"
            )
    ...
    def _pass_by_pass_data_init(
        ...
        if x_min_hist_det is not None:
            if (
                x_max_hist_det is None
                or y_min_hist_det is None
                or y_max_hist_det is None
                or Dx_hist_det is None
            ):
                raise ValueError(
                    "x_min_hist_det is not None but one among x_max_hist_det, y_min_hist_det, y_max_hist_det, Dx_hist_det is None!"
                )
            ...
            self.xg_hist_det = np.arange(
                x_min_hist_det - Dx_hist_det,
                x_max_hist_det + Dx_hist_det + 0.1 * Dx_hist_det,
                Dx_hist_det,
                float,
            )
    ...
    def _pass_by_pass_data_save(self, MP_e, impact_man, beamtim, buildup_sim):
        ...
        if MP_e.N_mp > 0:
            ...
            if self.flag_hist_det:
                # print 'here 1'
                mask_det_hist_x = np.logical_and(
                    MP_e.x_mp[0 : MP_e.N_mp] > self.x_min_hist_det,
                    MP_e.x_mp[0 : MP_e.N_mp] < self.x_max_hist_det,
                )
                mask_det_hist_y = np.logical_and(
                    MP_e.y_mp[0 : MP_e.N_mp] > self.y_min_hist_det,
                    MP_e.y_mp[0 : MP_e.N_mp] < self.y_max_hist_det,
                )
                ...
                if np.sum(mask_det_hist) > 0:
                    # print 'here 2'
                    histf.compute_hist(
                        MP_e.x_mp[0 : MP_e.N_mp][mask_det_hist],
                        MP_e.nel_mp[0 : MP_e.N_mp][mask_det_hist],
                        self.bias_x_hist_det,
                        self.Dx_hist_det,
                        self.nel_hist_det_line,
                    )
        ...
        if self.extract_ene_dist:
            ...
            for etypn in list(self.emit_ene_dist_test.keys()):
                if "hist" not in etypn:
                    saved_dict[
                        "emit_ene_test_%s_mat" % etypn
                    ] = self.emit_ene_dist_test[etypn]
    ...
    def _checkpoint_init(self, checkpoint_DT, checkpoint_folder):
        ...
        if checkpoint_DT is None:
            ...
            if checkpoint_folder is None:
                raise ValueError(
                    "checkpoint_folder not specified in simulation_parameters.input"
                )
    ...
    def _checkpoint_save(
        ...
        if self.flag_save_checkpoint:
            if beamtim.tt_curr - self.t_last_checkp >= self.checkpoint_DT:
                ...
                outpath = self.checkpoint_folder + "simulation_checkpoint_%d.pkl" % (
                    self.i_checkp
                )

                self._sim_state_single_save(
                    beamtim,
                    spacech_ele,
                    t_sc_ON,
                    flag_presence_sec_beams,
                ...
                if self.copy_main_outp_folder is not None:
                    self._copy_main_outp_to_safety(
                        outpath=self.copy_main_outp_folder, beamtim=beamtim
                    )
                ...
                if self.i_checkp > 0:
                    if self.flag_last_cloud:
                        prevpath = (
                            ...
                            + "simulation_checkpoint_%d.pkl" % (self.i_checkp - 1)
                        )
                        os.remove(prevpath)
    ...
    def _copy_main_outp_init(self, copy_main_outp_DT, copy_main_outp_folder):
        ...
        if copy_main_outp_folder is not None:
            ...
            raise ValueError(
                "copy_main_outp_folder not specified in simulation_parameters.input"
            )
    ...
    def load_from_output(self, last_t=None):
        ...
        saved_every_passage_list = [
            ...
            "nel_impact_hist_tot",
            "cos_angle_hist",
            "t_hist" "lifetime_hist" "t_lifetime_hist",
        ]

        not_time_dependent_list = [
            "xg_hist",
            "xg_hist_det",
            "En_g_hist",
            "b_spac",
        ...
        if "t_sc_video" in list(dict_history.keys()):
            dict_restored["t_sc_video"] = dict_history["t_sc_video"][
                : idx_t_sc_video + 1
            ].tolist()
    ...
    def _stepbystep_data_init(
        ...
        self.Nel_timep = 0.0 * self.t
        self.Nel_imp_time = 0.0 * self.t
        self.Nel_emit_time = 0.0 * self.t
        self.En_imp_eV_time = 0.0 * self.t
        self.En_emit_eV_time = 0.0 * self.t
        self.En_kin_eV_time = 0.0 * self.t
        self.cen_density = 0.0 * self.t

        if self.flag_detailed_MP_info == 1:
            self.N_mp_time = 0.0 * self.t
            self.nel_mp_ref_time = 0.0 * self.t
        else:
            self.N_mp_time = -1
            self.nel_mp_ref_time = -1

        if self.flag_cross_ion:
            self.Nel_cross_ion = 0.0 * self.t
        ...
        if self.flag_electric_energy:
            self.En_electric_eV_time = 0.0 * self.t
        else:
        ...
        if len(el_density_probes) > 0:
            ...
            self.el_dens_at_probes = np.zeros(
                (self.N_el_dens_probes, len(self.t))
            )  # to be changed
    ...
    def _stepbystep_data_save(self, impact_man, MP_e, beamtim, buildup_sim, cross_ion):
        ...
        if beamtim.tt_curr - self.t_last_save >= self.Dt_save:
            ...
            self.En_kin_eV_time[self.i_last_save] = np.sum(
                0.5
                * MP_e.mass
                / qe
                * MP_e.nel_mp[0 : MP_e.N_mp]
                * (
                    MP_e.vx_mp[0 : MP_e.N_mp] * MP_e.vx_mp[0 : MP_e.N_mp]
                    + MP_e.vy_mp[0 : MP_e.N_mp] * MP_e.vy_mp[0 : MP_e.N_mp]
                    + MP_e.vz_mp[0 : MP_e.N_mp] * MP_e.vz_mp[0 : MP_e.N_mp]
                )
            )

            if self.flag_electric_energy:
                self.En_electric_eV_time[
                    self.i_last_save
                ] = buildup_sim.spacech_ele.get_potential_electric_energy()

            flag_center = (MP_e.x_mp ** 2 + MP_e.y_mp ** 2) < self.r_center ** 2
            flag_center[MP_e.N_mp :] = False
            self.cen_density[self.i_last_save] = np.sum(MP_e.nel_mp[flag_center]) / (
                np.pi * self.r_center * self.r_center
            )

            if self.flag_el_dens_probes:
                for ii in range(self.N_el_dens_probes):
                    flag_center = (
                        (MP_e.x_mp - self.x_el_dens_probes[ii]) ** 2
                        + (MP_e.y_mp - self.y_el_dens_probes[ii]) ** 2
                    ) < self.r_el_dens_probes[ii] ** 2
                    flag_center[MP_e.N_mp :] = False
                    self.el_dens_at_probes[ii, self.i_last_save] = np.sum(
                        MP_e.nel_mp[flag_center]
                    ) / (np.pi * self.r_el_dens_probes[ii] ** 2)
            ...
            if self.flag_cross_ion:
                (
                    ...
                    self.DN_cross_ion[self.i_last_save],
                ) = cross_ion.save_cross_ion_data(self.cloud_name)

            if self.step_by_step_custom_observables is not None:
                for kk in list(self.step_by_step_custom_observables.keys()):
                    self.sbs_custom_data[kk][
                        self.i_last_save
                    ] = self.step_by_step_custom_observables[kk](buildup_sim)
    ...
    def _stepbystep_get_dict(self):
        ...
        if self.flag_detailed_MP_info == 1:
            ...
            dict_sbs_data["nel_mp_ref_time"] = self.nel_mp_ref_time[
                : self.i_last_save + 1
            ]

        if self.flag_cross_ion:
            dict_sbs_data["Nel_cross_ion"] = self.Nel_cross_ion[: self.i_last_save + 1]
            dict_sbs_data["N_mp_cross_ion"] = self.N_mp_cross_ion[
                : self.i_last_save + 1
            ]
            dict_sbs_data["DN_cross_ion"] = self.DN_cross_ion[: self.i_last_save + 1]

        if self.flag_electric_energy:
            dict_sbs_data["En_electric_eV_time"] = self.En_electric_eV_time[
                : self.i_last_save + 1
            ]

        if self.flag_el_dens_probes:
            dict_sbs_data["el_dens_at_probes"] = self.el_dens_at_probes[
                :, : self.i_last_save
            ]
            ...
            if (MP_e.N_mp > 0) and (self.i_obs < self.N_obs):
                if beamtim.tt_curr >= self.t_obs[self.i_obs]:
                    if self.flag_multiple_clouds:
                        filename_MP_state = "MP_state_%s_%d" % (
                            self.cloud_name,
                            self.i_obs,
                        )
                    else:
                    ...
                    sio.savemat(
                        path_MP_state,
                        {
                            "tt": beamtim.tt_curr,
                            "N_mp": MP_e.N_mp,
                            "x_mp": MP_e.x_mp[0 : MP_e.N_mp],
                            "y_mp": MP_e.y_mp[0 : MP_e.N_mp],
                            "z_mp": MP_e.z_mp[0 : MP_e.N_mp],
                            "vx_mp": MP_e.vx_mp[0 : MP_e.N_mp],
                            "vy_mp": MP_e.vy_mp[0 : MP_e.N_mp],
                            "vz_mp": MP_e.vz_mp[0 : MP_e.N_mp],
    ...
    def _sim_state_init(self, save_simulation_state_time_file):
        ...
        if save_simulation_state_time_file is None:
            ...
            type(save_simulation_state_time_file) is int
            and save_simulation_state_time_file == -1
        ):
            self.flag_save_simulation_state = False
        else:
            self.flag_save_simulation_state = True
            if type(save_simulation_state_time_file) is str:
                dict_save_simulation_state_time = sio.loadmat(
                    save_simulation_state_time_file
                )
                self.t_obs_sim = np.squeeze(
                    dict_save_simulation_state_time["t_obs"].real
                )
            else:
        ...
        if self.flag_save_simulation_state:
            if self.i_obs_sim < self.N_obs_sim:
                if beamtim.tt_curr >= self.t_obs_sim[self.i_obs_sim]:
                    filename_simulation_state = "simulation_state_%d.pkl" % (
                        self.i_obs_sim
                    )
    ...
    def _rho_video_save(self, spacech_ele, beamtim, rho_cloud):
        ...
        if self.flag_video and self.flag_last_cloud:
            ...
            if beamtim.flag_new_bunch_pass:
                ...
                filename_rho = self.folder_outp + "/rho_video/rho_pass%d.mat" % (
                    beamtim.pass_numb - 1
                )
                ...
                sio.savemat(
                    filename_rho,
                    {
                        "xg_sc": spacech_ele.xg,
                        "yg_sc": spacech_ele.yg,
                        "t_video": self.t_video,
                        "rho_video": self.rho_video,
                    },
                    oned_as="row",
                )
                print("Done")
        ...
        if self.flag_video and self.flag_multiple_clouds:
            if not os.path.exists(
                self.folder_outp + "/rho_video_%s" % (self.cloud_name)
            ):
            ...
            if beamtim.flag_new_bunch_pass:
                ...
                filename_rho = self.folder_outp + "rho_video_%s/rho_pass%d.mat" % (
                    self.cloud_name,
                    beamtim.pass_numb - 1,
                )
                print("Saving %s" % filename_rho)
                sio.savemat(
                    filename_rho,
                    {
                        "xg_sc": spacech_ele.xg,
                        "yg_sc": spacech_ele.yg,
                        "t_video": self.t_video_cloud,
                        "rho_video": self.rho_video_cloud,
                    },
                    oned_as="row",
                )
                print("Done")
    ...
    def _sc_video_save(self, spacech_ele, beamtim):
        ...
        if self.flag_sc_video:
            ...
            if beamtim.flag_new_bunch_pass:
                ...
                filename_efield = (
                    self.folder_outp
                    + "/efield_video/efield_pass%d.mat" % (beamtim.pass_numb - 1)
                )
                ...
                sio.savemat(
                    filename_efield,
                    {
                        "xg_sc": spacech_ele.xg,
                        "yg_sc": spacech_ele.yg,
                        "t_efield_video": self.t_efield_video,
                        "efx_video": self.efx_video,
                        "efy_video": self.efy_video,
                    },
                    oned_as="row",
                )
                print("Done")
    ...
    def _logfile_progressfile_stofile(self, beamtim, MP_e):
        ...
        string_tolog = timestr + (
            " pass. %d/%d, cloud=%s: Nel_tot=%e N_mp=%d\n"
            % (
                beamtim.pass_numb,
                beamtim.N_pass_tot,
                self.cloud_name,
                np.sum(MP_e.nel_mp[0 : MP_e.N_mp]),
                MP_e.N_mp,
            )
        )
        ...
        if self.progress_path is not None:
            try:
                with open(self.progress_path, "w") as flog:
                    flog.write(
                        ("%f" % (float(beamtim.pass_numb) / float(beamtim.N_pass_tot)))
                    )
            except IOError as err:
                print("Got: ", err)
    ...
    def _energy_cos_and_lifetime_angle_hist_init(
        self,
        Dt_En_hist,
        flag_cos_angle_hist,
        cos_angle_width,
        ...
        if flag_cos_angle_hist:
            N_angles = int(1.0 / cos_angle_width) + 1
            self.cos_angle_hist = []
            self.xg_hist_cos_angle = np.linspace(0.0, 1.0, N_angles)
        else:
    ...
    def _energy_cos_and_lifetime_angle_hist_save(self, beamtim, impact_man, MP_e):
        ...
        if beamtim.tt_curr >= self.t_last_En_hist + self.Dt_En_hist or np.isclose(
            beamtim.tt_curr,
            self.t_last_En_hist + self.Dt_En_hist,
            rtol=1.0e-10,
            atol=0.0,
        ):
            ...
            N_mp = MP_e.N_mp
            v_mod_square = (
                MP_e.vx_mp[:N_mp] ** 2 + MP_e.vy_mp[:N_mp] ** 2 + MP_e.vz_mp[:N_mp] ** 2
            )
            Ekin = 0.5 * MP_e.mass / qe * v_mod_square
            ekin_hist = np.zeros(impact_man.Nbin_En_hist, float)
            if N_mp > 0:
                histf.compute_hist(
                    Ekin, MP_e.nel_mp[:N_mp], 0, impact_man.DEn_hist, ekin_hist
                )
            ...
            if impact_man.flag_seg:
                if impact_man.flag_En_hist_seg:
                    for iseg in range(impact_man.chamb.N_vert):
                        self.En_hist_seg[iseg].append(
                            impact_man.seg_En_hist_lines[iseg].copy()
                        )
        ...
        if self.flag_lifetime_hist:
            if (
                beamtim.tt_curr >= self.t_last_lifetime_hist + self.Dt_lifetime_hist
                or np.isclose(
                    beamtim.tt_curr,
                    self.t_last_lifetime_hist + self.Dt_lifetime_hist,
                    rtol=1.0e-10,
                    atol=0.0,
                )
            ):
