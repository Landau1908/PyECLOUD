...
def monitorh5list_to_dict(
    filename_list, key="Bunch", flag_transpose=False, permissive=False
):
        ...
        try:
            ...
            if flag_transpose:
                for kk in list(monitor_dict.keys()):
                    monitor_dict[kk] = np.array(
                        list(monitor_dict[kk].T) + list(monitor_dict_curr[kk].T)
                    ).T
            else:
                for kk in list(monitor_dict.keys()):
                    monitor_dict[kk] = np.array(
                        list(monitor_dict[kk]) + list(monitor_dict_curr[kk])
                    )
...
def monitorh5list_to_obj(
    filename_list, key="Bunch", flag_transpose=False, permissive=False
):
    return obj_from_dict(
        monitorh5list_to_dict(filename_list, key, flag_transpose, permissive)
    )
