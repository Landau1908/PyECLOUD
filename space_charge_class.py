...
class space_charge:
    ...
    def __init__(
        ...
        if PyPICmode == "FiniteDifferences_ShortleyWeller":
            ...
            if len(np.atleast_1d(Dh)) == 2:
                self.PyPICobj = PIC_FFT_Open.FFT_OpenBoundary(
                    x_aper=chamb.x_aper, y_aper=chamb.y_aper, dx=Dh[0], dy=Dh[1]
                )
            else:
                self.PyPICobj = PIC_FFT_Open.FFT_OpenBoundary(
                    x_aper=chamb.x_aper, y_aper=chamb.y_aper, Dh=Dh
        ...
        self.Dt_sc = Dt_sc
        self.t_last_recom = 0.0

        self.U_sc_eV_stp = 0.0
    ...
    def recompute_spchg_efield(self, MP_e, flag_solve=True, flag_reset=True):
        ...
        if flag_solve:
            ...
            if self.comm is not None:
                ...
                if myid == 0:
                    avgrho = np.mean(recvbuf.reshape(n_sims, len(sendbuf)), axis=0)
                else:
    ...
    def get_sc_eletric_field(self, MP_e):
        Ex_sc_n, Ey_sc_n = self.PyPICobj.gather(
