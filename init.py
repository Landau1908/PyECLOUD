...
def read_parameter_files(pyecl_input_folder="./", skip_beam_files=False):
    ...
    input_parameters = inp_spec.import_module_from_file(
        "simulation_parameters", os.path.join(pyecl_input_folder, simulation_param_file)
    )

    # Get names of other input files
    machine_param_file = input_parameters.machine_param_file
    secondary_emission_parameters_file = (
        input_parameters.secondary_emission_parameters_file
    )
    beam_parameters_file = input_parameters.beam_parameters_file

    # Parse other input files
    machine_parameters = inp_spec.import_module_from_file(
        "machine_parameters", os.path.join(pyecl_input_folder, machine_param_file)
    )
    secondary_emission_parameters = inp_spec.import_module_from_file(
        "secondary_emission_parameters",
        os.path.join(pyecl_input_folder, secondary_emission_parameters_file),
    )
    ...
    inp_spec.assert_module_has_parameters(
        input_parameters, "combined_simulations_secondaryEmission_machine_parameters"
    )

    # Create config_dict with all allowed input parameters (not specified are set to default)
    config_dict = {}
    inp_spec.update_config_dict(
        config_dict,
        input_parameters,
    ...
    if not skip_beam_files:
        beam_beam = inp_spec.import_module_from_file(
            "beam_beam", os.path.join(pyecl_input_folder, beam_parameters_file)
        )
...
def read_input_files_and_init_components(
    ...
    **kwargs
    ...
    for attr, value in list(kwargs.items()):
        ...
        if attr in config_dict:
            ...
            raise inp_spec.PyECLOUD_ConfigException(
                "What exactly does %s do? It is not an expected input." % attr
            )
    ...
    if not skip_beam:
        ...
        b_par = pbf.beam_descr_from_fil(
            os.path.join(pyecl_input_folder, cc.beam_parameters_file),
            cc.betafx,
            cc.Dx,
            cc.betafy,
            cc.Dy,
        ...
        if flag_presence_sec_beams:
            for sec_b_file in cc.secondary_beams_file_list:
                sec_b_par_list.append(
                    pbf.beam_descr_from_fil(
                        os.path.join(pyecl_input_folder, sec_b_file),
                        cc.betafx,
                        cc.Dx,
                        cc.betafy,
                        cc.Dy,
    ...
    cloud_par_list.append(
        pcf.cloud_descr_from_file(cloudfilename=None, default_param_obj=cc)
    )
    if flag_multiple_clouds:
        ...
        for add_cloud_file in cc.additional_clouds_file_list:
            cloud_par_list.append(
                pcf.cloud_descr_from_file(
                    cloudfilename=os.path.join(pyecl_input_folder, add_cloud_file),
                    default_param_obj=cc,
                )
            )
    ...
    for cloud_par in cloud_par_list:
        if (
            cloud_par.cc.switch_model == "ECLOUD_nunif"
            or cloud_par.cc.switch_model == "ECLOUD_nunif_charging"
        ):
    ...
    if cc.chamb_type == "ellip":
        chamb = ellip_cham_geom_object(
            cc.x_aper, cc.y_aper, flag_verbose_file=cc.flag_verbose_file
        )
        ...
        chamb = gipfi.polyg_cham_geom_object(
            filename_chm_path, flag_non_unif_sey, **chamber_kwargs
        )
    elif cc.chamb_type == "rect":
        chamb = girfi.rect_cham_geom_object(
            cc.x_aper, cc.y_aper, flag_non_unif_sey, **chamber_kwargs
        )
    else:
        raise inp_spec.PyECLOUD_ConfigException(
    ...
    if not skip_beam:
        ...
        try:
            if os.path.isfile(pyecl_input_folder + "/" + b_par.beam_long_prof_file):
                beam_long_prof_file_path = (
                    pyecl_input_folder + "/" + b_par.beam_long_prof_file
                )
            elif os.path.isfile(
                pyecl_input_folder + "/" + b_par.beam_long_prof_file + ".mat"
            ):
                beam_long_prof_file_path = (
                    pyecl_input_folder + "/" + b_par.beam_long_prof_file + ".mat"
                )
        ...
        beamtim = beatim.beam_and_timing(
            b_par.flag_bunched_beam,
            b_par.fact_beam,
            b_par.coast_dens,
            b_par.q_part,
            b_par.beam_field_file,
            cc.lam_th,
            b_spac=b_par.b_spac,
            sigmaz=b_par.sigmaz,
            t_offs=b_par.t_offs,
            filling_pattern_file=b_par.filling_pattern_file,
            Dt=cc.Dt,
            t_end=cc.t_end,
            beam_long_prof_file=beam_long_prof_file_path,
            Dh_beam_field=b_par.Dh_beam_field,
            f_telescope_beam=b_par.f_telescope_beam,
            target_grid_beam=b_par.target_grid_beam,
            N_nodes_discard_beam=b_par.N_nodes_discard_beam,
            N_min_Dh_main_beam=b_par.N_min_Dh_main_beam,
            chamb=chamb,
            sigmax=b_par.sigmax,
            sigmay=b_par.sigmay,
            x_beam_pos=b_par.x_beam_pos,
            y_beam_pos=b_par.y_beam_pos,
            save_beam_field_file_as=b_par.save_beam_field_file_as,
            Nx=b_par.Nx,
            Ny=b_par.Ny,
            nimag=b_par.nimag,
            progress_mapgen_file=progress_mapgen_file,
        )
        ...
        if flag_presence_sec_beams:
            ...
            for ii in range(N_sec_beams):
                ...
                try:
                    if os.path.isfile(
                        pyecl_input_folder + "/" + sb_par.beam_long_prof_file
                    ):
                        sbeam_long_prof_file_path = (
                            pyecl_input_folder + "/" + sb_par.beam_long_prof_file
                        )
                    elif os.path.isfile(
                        pyecl_input_folder + "/" + b_par.beam_long_prof_file + ".mat"
                ...
                sec_beams_list.append(
                    beatim.beam_and_timing(
                        sb_par.flag_bunched_beam,
                        sb_par.fact_beam,
                        sb_par.coast_dens,
                        sb_par.q_part,
                        sb_par.beam_field_file,
                        cc.lam_th,
                        b_spac=sb_par.b_spac,
                        sigmaz=sb_par.sigmaz,
                        t_offs=sb_par.t_offs,
                        filling_pattern_file=sb_par.filling_pattern_file,
                        Dt=cc.Dt,
                        t_end=cc.t_end,
                        beam_long_prof_file=sbeam_long_prof_file_path,
                        Dh_beam_field=sb_par.Dh_beam_field,
                        f_telescope_beam=sb_par.f_telescope_beam,
                        target_grid_beam=sb_par.target_grid_beam,
                        N_nodes_discard_beam=sb_par.N_nodes_discard_beam,
                        N_min_Dh_main_beam=sb_par.N_min_Dh_main_beam,
                        chamb=chamb,
                        sigmax=sb_par.sigmax,
                        sigmay=sb_par.sigmay,
                        x_beam_pos=sb_par.x_beam_pos,
                        y_beam_pos=sb_par.y_beam_pos,
                        save_beam_field_file_as=sb_par.save_beam_field_file_as,
                        flag_secodary_beam=True,
                        t_primary_beam=beamtim.t,
                        Nx=sb_par.Nx,
                        Ny=sb_par.Ny,
                        nimag=sb_par.nimag,
                        progress_mapgen_file=(
                            cc.progress_path + ("_mapgen_sec_%d" % ii)
                        ),
                    )
                )
    else:
    ...
    if skip_spacech_ele:
        ...
        if cc.flag_em_tracking:
            if skip_beam:
                raise ValueError(
                    "Beam must be included when using electromagnetic tracking!!"
                )

            spacech_ele_sim = scc_em.space_charge_electromagnetic(
                chamb,
                cc.Dh_sc,
                b_par.gamma_rel,
                Dt_sc=cc.Dt_sc,
                sparse_solver=cc.sparse_solver,
                PyPICmode=cc.PyPICmode,
                f_telescope=cc.f_telescope,
                target_grid=cc.target_grid,
                N_nodes_discard=cc.N_nodes_discard,
                N_min_Dh_main=cc.N_min_Dh_main,
                Dh_U_eV=cc.Dh_electric_energy,
            )
        else:
            spacech_ele_sim = scc.space_charge(
                chamb,
                cc.Dh_sc,
                Dt_sc=cc.Dt_sc,
                sparse_solver=cc.sparse_solver,
    ...
    for cloud_par in cloud_par_list:
        ...
        MP_e = MPs.MP_system(
            thiscloud.N_mp_max,
            thiscloud.nel_mp_ref_0,
            thiscloud.fact_split,
            thiscloud.fact_clean,
            thiscloud.N_mp_regen_low,
            thiscloud.N_mp_regen,
            thiscloud.N_mp_after_regen,
            thiscloud.Dx_hist,
            thiscloud.Nx_regen,
            thiscloud.Ny_regen,
            thiscloud.Nvx_regen,
            thiscloud.Nvy_regen,
            thiscloud.Nvz_regen,
            thiscloud.regen_hist_cut,
            chamb,
            N_mp_soft_regen=thiscloud.N_mp_soft_regen,
            N_mp_after_soft_regen=thiscloud.N_mp_after_soft_regen,
            N_mp_async_regen=thiscloud.N_mp_async_regen,
            N_mp_after_async_regen=thiscloud.N_mp_after_async_regen,
            charge=thiscloud.cloud_charge,
            mass=thiscloud.cloud_mass,
        ...
        if thiscloud.switch_model == "perfect_absorber":
            ...
            if thiscloud.s_param is not None:
                if thiscloud.switch_model == 0 or thiscloud.switch_model == "ECLOUD":
                    ...
                    raise inp_spec.PyECLOUD_ConfigException(
                        "s parameter can be changed only in the ECLOUD sec. emission model!"
                    )

            if thiscloud.switch_model in (0, "ECLOUD"):
                kwargs_secem[
                    "flag_costheta_delta_scale"
                ] = thiscloud.flag_costheta_delta_scale
                kwargs_secem[
                    "flag_costheta_Emax_shift"
                ] = thiscloud.flag_costheta_Emax_shift
                sey_mod = SEY_model_ECLOUD(
                    thiscloud.Emax,
                    thiscloud.del_max,
                    thiscloud.R0,
                    E_th=thiscloud.E_th,
                    sigmafit=thiscloud.sigmafit,
                    mufit=thiscloud.mufit,
                ...
                sey_mod = SEY_model_ECLOUD_non_unif(
                    chamb,
                    thiscloud.Emax,
                    thiscloud.del_max,
                    thiscloud.R0,
                ...
                sey_mod = SEY_model_ECLOUD_non_unif_charging(
                    chamb,
                    thiscloud.Emax,
                    thiscloud.del_max,
                    thiscloud.R0,
                ...
                if os.path.isfile(pyecl_input_folder + "/" + thiscloud.sey_file):
                    ...
                    pyecl_input_folder + "/" + thiscloud.sey_file + ".mat"
                ):
                    sey_file_path = (
                        pyecl_input_folder + "/" + thiscloud.sey_file + ".mat"
                    )
        ...
        flag_seg = (
            thiscloud.flag_hist_impact_seg == 1
            or thiscloud.flag_hist_impact_seg is True
        )

        if flag_seg and cc.chamb_type == "ellip":
            print(
                "Warning: You cannot enable flag_hist_impact_seg for an ellip chamber --> disabled!"
            )
            flag_seg = False

        if cc.flag_lifetime_hist:
            if (
                cc.Nbin_lifetime_hist is None
                or cc.lifetime_hist_max is None
                or cc.Dt_lifetime_hist is None
            ):
                raise inp_spec.PyECLOUD_ConfigException(
        ...
        impact_man = imc.impact_management(
            ...
            sey_mod,
            thiscloud.Dx_hist,
            thiscloud.scrub_en_th,
            cc.Nbin_En_hist,
            cc.En_hist_max,
            cc.Nbin_lifetime_hist,
            cc.lifetime_hist_max,
            cc.flag_lifetime_hist,
            flag_seg=flag_seg,
            flag_En_hist_seg=thiscloud.flag_En_hist_seg,
            cos_angle_width=cc.cos_angle_width,
        )

        # Init gas ionization and photoemission
        if thiscloud.gas_ion_flag == 1:
            resgasion = gic.residual_gas_ionization(
                thiscloud.unif_frac,
                thiscloud.P_nTorr,
                thiscloud.sigma_ion_MBarn,
                thiscloud.Temp_K,
                chamb,
                thiscloud.E_init_ion,
                thiscloud.flag_lifetime_hist,
            )
        else:
            resgasion = None

        if thiscloud.photoem_flag == 1:
            phemiss = gpc.photoemission(
                thiscloud.inv_CDF_refl_photoem_file,
                thiscloud.k_pe_st,
                thiscloud.refl_frac,
                thiscloud.e_pe_sigma,
                thiscloud.e_pe_max,
                thiscloud.alimit,
                thiscloud.x0_refl,
                thiscloud.y0_refl,
                thiscloud.out_radius,
                chamb,
                thiscloud.phem_resc_fac,
                thiscloud.energy_distribution,
                thiscloud.photoelectron_angle_distribution,
                beamtim,
                thiscloud.flag_continuous_emission,
            )
        elif thiscloud.photoem_flag in (2, "from_file"):
            phemiss = gpc.photoemission_from_file(
                thiscloud.inv_CDF_all_photoem_file,
                chamb,
                thiscloud.phem_resc_fac,
                thiscloud.energy_distribution,
                thiscloud.e_pe_sigma,
            ...
            if os.path.isfile(
                pyecl_input_folder + "/" + thiscloud.filename_chm_photoem
            ):
                filename_chm_photoem_path = (
                    pyecl_input_folder + "/" + thiscloud.filename_chm_photoem
                )
            elif os.path.isfile(
                pyecl_input_folder + "/" + thiscloud.filename_chm_photoem + ".mat"
            ):
                filename_chm_photoem_path = (
                    pyecl_input_folder + "/" + thiscloud.filename_chm_photoem + ".mat"
                )
            ...
            if not chamb_phemiss.vertexes_are_subset(chamb):
                raise gipfi.PyECLOUD_ChamberException(
                    "Chambers for secondary emission and photoemission do not have the same shape!"
                )
            phemiss = gpc.photoemission_per_segment(
                chamb_phemiss,
                thiscloud.energy_distribution,
                thiscloud.e_pe_sigma,
                thiscloud.e_pe_max,
        ...
        if not skip_pyeclsaver:
            ...
            pyeclsaver.start_observing(
                cc.Dt,
                MP_e,
                beamtim,
                impact_man,
                thiscloud.r_center,
                thiscloud.Dt_En_hist,
                thiscloud.logfile_path,
                thiscloud.progress_path,
                flag_detailed_MP_info=thiscloud.flag_detailed_MP_info,
                flag_movie=thiscloud.flag_movie,
                flag_sc_movie=thiscloud.flag_sc_movie,
                save_mp_state_time_file=thiscloud.save_mp_state_time_file,
                flag_presence_sec_beams=flag_presence_sec_beams,
                sec_beams_list=sec_beams_list,
                dec_fac_secbeam_prof=thiscloud.dec_fac_secbeam_prof,
                el_density_probes=thiscloud.el_density_probes,
                save_simulation_state_time_file=thiscloud.save_simulation_state_time_file,
                x_min_hist_det=thiscloud.x_min_hist_det,
                x_max_hist_det=thiscloud.x_max_hist_det,
                y_min_hist_det=thiscloud.y_min_hist_det,
                y_max_hist_det=thiscloud.y_max_hist_det,
                Dx_hist_det=thiscloud.Dx_hist_det,
                dec_fact_out=cc.dec_fact_out,
                stopfile=cc.stopfile,
                filen_main_outp=thiscloud.filen_main_outp,
                flag_cos_angle_hist=thiscloud.flag_cos_angle_hist,
                cos_angle_width=thiscloud.cos_angle_width,
                flag_multiple_clouds=flag_multiple_clouds,
                cloud_name=thiscloud.cloud_name,
                flag_last_cloud=flag_last_cloud,
                checkpoint_DT=cc.checkpoint_DT,
                checkpoint_folder=cc.checkpoint_folder,
                copy_main_outp_folder=cc.copy_main_outp_folder,
                copy_main_outp_DT=cc.copy_main_outp_DT,
                extract_sey=cc.extract_sey,
        ...
        if cc.track_method == "Boris":
            if cc.flag_em_tracking == True:
                raise ValueError(
                    "Track_method should be 'BorisMultipole' to use electromagnetic space charge!!"
                )
            dynamics = dynB.pusher_Boris(
                cc.Dt,
                cc.B0x,
                cc.B0y,
                cc.B0z,
            ...
            if not (
                np.abs(thiscloud.cloud_charge - (-qe)) / np.abs(qe) < 1e-3
                and np.abs(thiscloud.cloud_mass - m_e) / m_e < 1e-3
            ):
                raise ValueError(
                    "StrongBdip tracking method is implemented only for electrons!"
                )
            if cc.B == -1:
                B = (
                    2
            ...
            if not (
                ...
                raise ValueError(
                    "StrongBgen tracking method is implemented only for electrons!"
                )
            dynamics = dyngen.pusher_strong_B_generalized(
                cc.Dt, cc.B0x, cc.B0y, cc.B_map_file, cc.fact_Bmap, cc.B_zero_thrhld
            )
        elif cc.track_method == "BorisMultipole":
            dynamics = dynmul.pusher_Boris_multipole(
                Dt=cc.Dt,
                N_sub_steps=cc.N_sub_steps,
                B_multip=cc.B_multip,
                B_skew=cc.B_skew,
            )
        else:
            raise inp_spec.PyECLOUD_ConfigException(
                "track_method should be 'Boris' or 'StrongBdip' or 'StrongBgen' or 'BorisMultipole'"
            )

        # Initial electron density
        if thiscloud.init_unif_flag == 1:
            print(
                "Adding initial %.2e electrons to the initial distribution"
                % thiscloud.Nel_init_unif
            )
            MP_e.add_uniform_MP_distrib(
                thiscloud.Nel_init_unif,
                thiscloud.E_init_unif,
                thiscloud.x_max_init_unif,
                thiscloud.x_min_init_unif,
                thiscloud.y_max_init_unif,
                thiscloud.y_min_init_unif,
            )

        if thiscloud.init_unif_edens_flag == 1:
            print(
                "Adding initial %.2e electrons/m^3 to the initial distribution"
                % thiscloud.init_unif_edens
            )
            MP_e.add_uniform_ele_density(
                n_ele=thiscloud.init_unif_edens,
                E_init=thiscloud.E_init_unif_edens,
                x_max=thiscloud.x_max_init_unif_edens,
                x_min=thiscloud.x_min_init_unif_edens,
                y_max=thiscloud.y_max_init_unif_edens,
                y_min=thiscloud.y_min_init_unif_edens,
            )

        if (
            thiscloud.filename_init_MP_state != -1
            and thiscloud.filename_init_MP_state is not None
        ):
            print(
                "Adding initial electrons from: %s" % thiscloud.filename_init_MP_state
            )
            MP_e.add_from_file(thiscloud.filename_init_MP_state)

        # Init empty rho for cloud
        if hasattr(spacech_ele_sim, "rho"):
            rho = spacech_ele_sim.rho * 0.0
        else:
            rho = None

        cloud = cman.Cloud(
            thiscloud.cloud_name,
            thiscloud,
            MP_e,
            impact_man,
            dynamics,
            pyeclsaver,
            thiscloud.gas_ion_flag,
            resgasion,
            thiscloud.t_ion,
            thiscloud.photoem_flag,
            phemiss,
            rho,
        )
    ...
    if flag_cross_ion:
        cross_ion = cion.Cross_Ionization(
            pyecl_input_folder, cc.cross_ion_definitions, cloud_list, chamb.area
        )
    else:
        cross_ion = None
