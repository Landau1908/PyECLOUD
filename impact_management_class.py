...
class impact_management(object):
    def __init__(
        ...
        xg_hist = np.arange(0, chamb.x_aper + 2.0 * Dx_hist, Dx_hist, float)
        xgr_hist = xg_hist[1:]
        ...
        self.En_g_hist = np.linspace(0.0, En_hist_max, Nbin_En_hist)  # hist. grid
        ...
        if flag_lifetime_hist:
            ...
            self.lifetime_g_hist = np.linspace(
                0.0, lifetime_hist_max, Nbin_lifetime_hist
            )  # hist. grid
            # hist. step
            self.Dt_lifetime_hist = self.lifetime_g_hist[1] - self.lifetime_g_hist[0]
            ...
            if Nimpact > 0:
                ...
                if self.flag_lifetime_hist:
                    ...
                    if sum(MP_e.t_last_impact[flag_impact] > 0) > 0:
                        histf.compute_hist(
                            lifetime_impact[MP_e.t_last_impact[flag_impact] > 0],
                            nel_impact[MP_e.t_last_impact[flag_impact] > 0],
                            0.0,
                            Dt_lifetime_hist,
                            self.lifetime_hist_line,
                        )
                ...
                histf.compute_hist(
                    x_impact,
                    nel_impact * (E_impact_eV > scrub_en_th),
                    bias_x_hist,
                    Dx_hist,
                    ...
                    if self.flag_En_hist_seg:
                        for iseg in range(self.chamb.N_vert):
                            ...
                            if np.sum(mask_this_seg) > 0:
                                ...
                                En_imp_hist_this_seg[
                                    En_imp_hist_this_seg > En_hist_max
                                ] = En_hist_max
                                histf.compute_hist(
                                    En_imp_hist_this_seg,
                                    nel_impact[mask_this_seg],
                                    0.0,
                                    DEn_hist,
                ...
                histf.compute_hist(
                    En_imp_hist, nel_impact, 0.0, DEn_hist, self.En_hist_line
                )
        ...
        for i_ct, ct in enumerate(cos_theta_test):
            ...
            for i_ene, Ene in enumerate(E_impact_eV_test):
                ...
                (
                    ...
                    mass=mass,
                    nel_impact=nel_impact,
                    x_impact=nel_impact * 0,
                    y_impact=nel_impact * 0,
                    z_impact=nel_impact * 0,
                    vx_impact=vx * np.ones_like(nel_impact),
                    vy_impact=vy * np.ones_like(nel_impact),
                    vz_impact=nel_impact * 0,
                    Norm_x=np.ones_like(nel_impact),
                    Norm_y=np.zeros_like(nel_impact),
                ...
                for etype in list(self.sey_mod.event_types.keys()):
                    ...
                    thisdelta[i_ct, i_ene] = np.sum(
                        nel_emit_tot_events[event_type == etype]
                    ) / np.sum(nel_impact)
    ...
    def extract_energy_distributions(
        self,
        n_rep,
        E_impact_eV_test,
        cos_theta_test,
        ...
        emit_ene_g_hist = np.linspace(
            0.0, E_impact_eV_test * factor_ene_dist_max, Nbin_extract_ene
        )
        ...
        for i_ct, ct in enumerate(cos_theta_test):
            ...
            for etype in list(self.sey_mod.event_types.keys()):
                ...
                if E_all_MPs_eV[extended_event_type == etype].shape == (0,):
                    ...
                    histf.compute_hist(
                        E_all_MPs_eV[extended_event_type == etype],
                        np.ones(len(E_all_MPs_eV[extended_event_type == etype])),
                        0.0,
                        Dextract_ene,
                        temp,
                    )
