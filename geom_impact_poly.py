...
class polyg_cham_geom_object:
    ...
    def is_outside(self, x_mp, y_mp):
        ...
        if flag_outside.any():
            ...
            for ii in range(self.N_edg):
                flag_inside_chk[flag_inside_chk] = (
                    (y_mp_chk[flag_inside_chk] - self.Vy[ii])
                    * (self.Vx[ii + 1] - self.Vx[ii])
                    - (x_mp_chk[flag_inside_chk] - self.Vx[ii])
                    * (self.Vy[ii + 1] - self.Vy[ii])
                ) > 0
    ...
    def impact_point_and_normal(
        ...
        for ii in range(self.N_edg):
            t_curr = (
                self.Nx[ii] * (self.Vx[ii] - x_in) + self.Ny[ii] * (self.Vy[ii] - y_in)
            ...
            if mask_same_min.any():
                ...
                t_border = (
                    (y_out[mask_same_min] - y_in[mask_same_min])
                    * (x_in[mask_same_min] - self.Vx[ii])
                    + (x_in[mask_same_min] - x_out[mask_same_min])
                    * (y_in[mask_same_min] - self.Vy[ii])
                ) / (
                    (y_out[mask_same_min] - y_in[mask_same_min])
                    * (self.Vx[ii + 1] - self.Vx[ii])
                    + (x_in[mask_same_min] - x_out[mask_same_min])
                ...
                mask_upd_i_found[mask_same_min] = logical_and(
                    t_border >= 0.0, t_border <= 1.0
                )
                i_found[mask_upd_i_found] = ii

        t_min = resc_fac * t_min
        x_int = t_min * x_out + (1.0 - t_min) * x_in
        y_int = t_min * y_out + (1.0 - t_min) * y_in
        z_int = 0 * t_min
