...
def bunch_train4(t, b_spac, t_offs, ppb_vect, sigmaz_vect):
    ...
    for ii in range(0, N_bucket):
        if np.mod(ii, N_bucket / 20) == 0:
            print(
                (
                    "Beam profile generation %.0f" % (float(ii) / float(N_bucket) * 100)
                    + """%"""
                )
            )
        ...
        if sigmaz > 0:
            ...
            mask_to_be_updated = np.abs(zz - z0) < (10.0 * sigmaz)

            val[mask_to_be_updated] = val[mask_to_be_updated] + ppb / (
                sigmaz * np.sqrt(2 * np.pi)
            ) * np.exp(
                -(zz[mask_to_be_updated] - z0)
                * (zz[mask_to_be_updated] - z0)
                / (2 * sigmaz * sigmaz)
            )
...
class beam_and_timing:
    def __init__(
        ...
        if flag_bunched_beam:
            ...
            if type(filling_pattern_file) is str:
                if ".mat" in filling_pattern_file:
                    ...
                    raise ValueError(
                        "Input of filling scheme via txt files is discontinued!"
                    )
            ...
            if flag_secodary_beam:
                ...
                t = np.arange(0.0, N_bunches * b_spac + t_end + 2 * Dt, Dt)
            ...
            Dt_vect = np.diff(t)
            if (np.max(np.abs(Dt_vect)) - np.mean(np.abs(Dt_vect))) > 1e-4 * np.mean(
                np.abs(Dt_vect)
            ):
                flag_unif_Dt = False
            else:
                ...
                print(
                    "The time step Dt provided in simulation_parameters.input will be ignored."
                )
                Dt = t[1] - t[0]
        ...
        if flag_secodary_beam:
            if len(t) != len(t_primary_beam):
                raise ValueError(
                    "Time axes provided for primary and secondary beams should be identical!"
                )
        ...
        if beam_field_file == -1 or beam_field_file == "computeFD":
            ...
            if Dh_beam_field is None:
                raise ValueError(
                    "Grid size Dh_beam_field MUST be provided for beam field computation!"
                )

            from . import space_charge_class as scc
            from numpy import exp, pi

            scb = scc.space_charge(chamb, Dh_beam_field, Dt_sc=1.0)
            ...
            rho = (
                ...
                * (
                    sspe.erf(
                        (scb.xn - x_beam_pos) / (np.sqrt(2) * sigmax)
                        + Dh_beam_field / (2 * np.sqrt(2) * sigmax)
                    )
                    - sspe.erf(
                        (scb.xn - x_beam_pos) / (np.sqrt(2) * sigmax)
                        - Dh_beam_field / (2 * np.sqrt(2) * sigmax)
                ...
                * (
                    sspe.erf(
                        (scb.yn - y_beam_pos) / (np.sqrt(2) * sigmay)
                        + Dh_beam_field / (2 * np.sqrt(2) * sigmay)
                    )
                    - sspe.erf(
                        (scb.yn - y_beam_pos) / (np.sqrt(2) * sigmay)
                        - Dh_beam_field / (2 * np.sqrt(2) * sigmay)
            ...
            print(
                "No beam field file provided -> Calculate field using Bassetti Erskine formula"
            )

            if chamb.chamb_type != "ellip":
                raise ValueError(
                    "You can only use Bassetti Erskine formula with an elliptic chamber!"
                )
            if Nx is None or Ny is None or nimag is None:
                raise ValueError(
                    "Nx, Ny and nimag MUST be provided for Bassetti Erskine formula!"
                )
            if x_beam_pos != 0.0 or y_beam_pos != 0.0:
                raise ValueError(
                    "x_beam_pos, y_beam_pos and MUST be 0 for Bassetti Erskine formula!"
                )

            print(
                "sigmax=%.3e, sigmay=%.3e, Nx=%d, Ny=%d, nimag=%d"
            ...
            for ii in range(len(xx)):
                ...
                if np.mod(ii, Nx / 20) == 0:
                    print(
                        (
                            "Beam field map generation %.0f"
                            % (float(ii) / float(Nx) * 100)
                            + """%"""
                        )
                    )
            ...
            if f_telescope_beam is None:
                raise ValueError(
                    "Aspect ratio MUST be provided for multigrid beam field computation!"
                )
            if target_grid_beam is None:
                raise ValueError(
                    "Target grid MUST be provided for multigrid beam field computation!"
                )
            if N_nodes_discard_beam is None:
                raise ValueError(
                    " N_nodes_discard_beam MUST be provided for multigrid beam field computation!"
                )
            if N_min_Dh_main_beam is None:
                raise ValueError(
                    " N_min_Dh_main_beam MUST be provided for multigrid beam field computation!"
                )
            ...
            PyPICmain = PIC_FDSW.FiniteDifferences_ShortleyWeller_SquareGrid(
                chamb=chamb, Dh=Dh_beam_field, sparse_solver="PyKLU"
            )
            import PyPIC.MultiGrid as PIC_MG

            PyPICobj = PIC_MG.AddTelescopicGrids(
                pic_main=PyPICmain,
                f_telescope=f_telescope_beam,
                target_grid=target_grid_beam,
                N_nodes_discard=N_nodes_discard_beam,
                N_min_Dh_main=N_min_Dh_main_beam,
                sparse_solver="PyKLU",
            )
            ...
            PyPICmain.rho = (
                ...
                * (
                    sspe.erf(
                        (PyPICmain.xn - x_beam_pos) / (np.sqrt(2) * sigmax)
                        + Dh_beam_field / (2 * np.sqrt(2) * sigmax)
                    )
                    - sspe.erf(
                        (PyPICmain.xn - x_beam_pos) / (np.sqrt(2) * sigmax)
                ...
                * (
                    sspe.erf(
                        (PyPICmain.yn - y_beam_pos) / (np.sqrt(2) * sigmay)
                        + Dh_beam_field / (2 * np.sqrt(2) * sigmay)
                    )
                    - sspe.erf(
                        (PyPICmain.yn - y_beam_pos) / (np.sqrt(2) * sigmay)
            ...
            for pic_dual in PyPICobj.pic_list:
                ...
                pic.rho = (
                    ...
                    * (
                        sspe.erf(
                            (pic.xn - x_beam_pos) / (np.sqrt(2) * sigmax)
                            + dh / (2 * np.sqrt(2) * sigmax)
                        )
                        - sspe.erf(
                            (pic.xn - x_beam_pos) / (np.sqrt(2) * sigmax)
                            - dh / (2 * np.sqrt(2) * sigmax)
                    ...
                    * (
                        sspe.erf(
                            (pic.yn - y_beam_pos) / (np.sqrt(2) * sigmay)
                            + dh / (2 * np.sqrt(2) * sigmay)
                        )
                        - sspe.erf(
                            (pic.yn - y_beam_pos) / (np.sqrt(2) * sigmay)
                            - dh / (2 * np.sqrt(2) * sigmay)
        ...
        if save_beam_field_file_as is not None:
            if flag_PyPIC_state_mode:
                raise ValueError(
                    "You cannot save the field maps in multigrid mode! Sorry..."
                )
            sio.savemat(
        ...
        if not (flag_unif_Dt):
            ...
            print(
                "Dt provided in input will be used only as reference for savings and substeps."
            )
    ...
    def get_beam_eletric_field(self, MP_e):
        ...
        if (self.lam_t_curr > self.lam_th_beam_field) and (MP_e.N_mp > 0):
            ...
            Ex_n_beam, Ey_n_beam = iff.int_field(
