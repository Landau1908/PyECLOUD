...
class polyg_cham_geom_object(object):
    ...
    def is_outside(self, x_mp, y_mp):
        return self.cythonisoutside(
            x_mp, y_mp, self.Vx, self.Vy, self.cx, self.cy, self.N_edg
        )

    # @profile
    def impact_point_and_normal(
        self, x_in, y_in, z_in, x_out, y_out, z_out, resc_fac=0.99, flag_robust=True
    ):
        ...
        if sum(mask_found) < N_impacts:
            ...
            par_cross = arctan2(
                self.cx * y_in[mask_not_found], self.cy * x_int[mask_not_found]
            )
            ...
            neg_flag = (
                Nx_corr * x_int[mask_not_found] + Ny_corr * y_int[mask_not_found]
            ) > 0
        ...
        if flag_robust:
            ...
            if flag_impact.any():
                ...
                if self.flag_verbose_stdout:
                    print(
                        "Reporting backtrack error of kind 2: outside after backtracking"
                    )
                    print("x_in, y_in, x_out, y_out")
                    for i_err in range(N_errors):
                        lcurr = "%.10e,%.10e,%.10e,%.10e" % (
            ...
            if sum(flag_impact) > 0:
                ...
                if self.flag_verbose_stdout:
                    print(
                        "Reporting backtrack error of kind 3: outside after correction"
                    )
                    print("x_in, y_in, x_out, y_out")
                x_in_error = x_in[flag_impact]
                y_in_error = y_in[flag_impact]
    ...
    def vertex_is_on_edge(self, x, y):
        ...
        """
        for diff_x, diff_y, vx, vy in zip(
            np.diff(self.Vx), np.diff(self.Vy), self.Vx, self.Vy
        ):
    ...
    def vertexes_are_subset(self, other_chamb):
        ...
        """
        if not all(
            self.vertex_is_on_edge(x, y) for x, y in zip(other_chamb.Vx, other_chamb.Vy)
        ):
            return False
        if not all(
            other_chamb.vertex_is_on_edge(x, y) for x, y in zip(self.Vx, self.Vy)
        ):
            return False
        return True
...
class polyg_cham_photoemission(polyg_cham_geom_object):
    ...
    def __init__(self, filename_chm):
        ...
        if phem_cdf[-1] != 1:
            raise PyECLOUD_ChamberException(
                "phem_cdf of chamb_dict does not end with 1."
            )
        if np.any(np.diff(phem_cdf) < 0):
            raise PyECLOUD_ChamberException(
                "phem_cdf of chamb_dict is not monotonically increasing."
            )
    ...
    def get_photoelectron_positions(self, N_mp_gen):
        ...
        for i_seg, N_mp_seg in enumerate(N_mp_segment):
            if N_mp_seg != 0:
                ...
                self._get_photoelectron_position_segment(
                    N_mp_seg,
                    x_new_mp[N_mp_curr:N_mp_after],
                    y_new_mp[N_mp_curr:N_mp_after],
                    i_seg,
                )
    ...
    def _get_photoelectron_position_segment(self, N_mp, x_new_mp, y_new_mp, i_seg):
        ...
        if n_mp_outside != 0:
            ...
            print(
                "%i out of %i MPs were generated outside! -> recursion"
                % (n_mp_outside, N_mp)
            )

            # Because of advanced numpy array indexing, the output of this call has to be explicitly assigned.
            (
                x_new_mp[flag_outside],
                y_new_mp[flag_outside],
            ) = self._get_photoelectron_position_segment(
                n_mp_outside, x_new_mp[flag_outside], y_new_mp[flag_outside], i_seg
            )
